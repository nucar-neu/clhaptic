...
int main(int argc, char** argv) {
   ...
   for(int j = 0; j < num_phases; j++ )
   {
	   ...
	   //j is just a unique identifier for a phase, could possibly remove
	   //it and replace it with a random no generator and something like a dictionary
	   eventList->markPhase(j);
	   //eventList->analysePhaseChange();
   }
   for(int k = 0; k < 10; k++) {
     printf("C[%d] = %d\n", k, C[k]); 
   }
